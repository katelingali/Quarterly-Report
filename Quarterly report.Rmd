...
crab <- crab %>% #Selecting and renaming columns
  select('DCLS - ID confirmation', 'Date received at DCLS', 'KPC', 'NDM', 'VIM', 'OXA-48', 'IMP', 'OXA-23', 'OXA-24', 'OXA-58') %>%
...
bar <- ggplot(cre_pcr_pos, aes(y = PCR, fill = Result, na.rm=TRUE)) + 
  ...
  geom_text(stat = 'count', aes(group = Result, label = ..count..), 
            position = position_stack(reverse = TRUE, vjust = 0.5), colour = "black") + 
...
crab_pcr <- quarter_ %>%
  filter(Genus == 'CRAB') %>%
  gather('KPC','NDM','VIM','OXA-48','IMP', 'OXA-23', 'OXA-24', 'OXA-58', key = 'PCR', value = 'Result')
...
bar
```
OXA PCR (OXA-23, OXA-24, OXA-58) run on all CRAB isolates starting 06/01/2022. OXA PCR only run on cluster isolates before 06/01/2022. Several results still pending due to a delay at Maryland Public Health Laboratory.
